...
namespace AppDCASeparacao
{
    ...
    public class WebService1 : System.Web.Services.WebService
    {
        ...
        public List<Faturamento> ListaPedidosParaFaturar()
        {
            ...
            {
                cnn.Open();
                OracleCommand cmd = new OracleCommand("SELECT NUMCAR, COUNT(DISTINCT NUMPED)QTPEDIDOS, SUM(NUMITENS)QTITENS, SUM((SELECT COUNT(DISTINCT NUMPED)EM_ABERTO FROM PCPEDC WHERE C.NUMPED = PCPEDC.NUMPED AND PCPEDC.POSICAO = 'M' AND PCPEDC.ORDEMCONF IS NOT NULL AND PCPEDC.DATA > TRUNC(SYSDATE) - 60 AND PCPEDC.DTFINALSEP IS NULL))PED_PENDENTE, " +
                    " SUM((SELECT COUNT(DISTINCT NUMPED)FINALIZADO FROM PCPEDC WHERE C.NUMPED = PCPEDC.NUMPED  AND PCPEDC.POSICAO = 'M' AND PCPEDC.ORDEMCONF IS NOT NULL AND PCPEDC.DATA > TRUNC(SYSDATE) - 60 AND PCPEDC.DTFINALSEP IS NOT NULL))PED_FINALIZADO , " +
                    " DECODE((COUNT(DISTINCT NUMPED) - SUM((SELECT COUNT(DISTINCT NUMPED)FINALIZADO FROM PCPEDC WHERE C.NUMPED = PCPEDC.NUMPED  AND PCPEDC.POSICAO = 'M' AND PCPEDC.ORDEMCONF IS NOT NULL AND PCPEDC.DATA > TRUNC(SYSDATE) - 60 AND PCPEDC.DTFINALSEP IS NOT NULL))),'0','SIM','NAO')FATURAR " +
